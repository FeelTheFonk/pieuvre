...
pub fn run(profile: &str) -> Result<()> {
    ...
    println!(
        ...
        report.hardware.cpu.logical_cores, report.hardware.cpu.physical_cores
    );
    if report.hardware.cpu.is_hybrid {
        println!(
            "  Hybrid:     [OK] {} P-Cores + {} E-Cores",
            report.hardware.cpu.p_cores.len(),
            report.hardware.cpu.e_cores.len()
        );
    }
    println!(
        "  RAM:        {:.1} GB total / {:.1} GB disponible",
    ...
    println!("                       TÉLÉMÉTRIE");
    println!("═══════════════════════════════════════════════════════════════════");
    let diag_status = if report.telemetry.diagtrack_enabled {
        "[ACTIF]"
    } else {
        "[OFF]"
    };
    ...
    let adv_status = if report.telemetry.advertising_id_enabled {
        "[ACTIF]"
    } else {
        "[OFF]"
    };
    ...
    let power_optimized = power_plan.contains("High")
        || power_plan.contains("Ultimate")
        || power_plan.contains("Bitsum");
    ...
    match profile {
        "gaming" => {
            ...
            if report.hardware.cpu.is_hybrid {
                ...
                if priority_sep != 0x26 {
                    recommendations.push((
                        "PERF",
                        "Optimiser scheduler pour CPU hybrid (P-Core priority)",
                    ));
                }
            }
            ...
            if !timer_optimized {
                if is_laptop {
                    recommendations.push((
                        "WARN",
                        "Timer 0.5ms - Attention: +25% conso batterie sur laptop",
                    ));
                } else {
            ...
            if !power_optimized {
                if is_laptop {
                    recommendations.push((
                        "WARN",
                        "Power Plan - High Performance recommande (pas Ultimate)",
                    ));
                } else {
        ...
        _ => {
            recommendations.push((
                "INFO",
                "Profil non reconnu, utilisez: gaming, privacy, workstation",
            ));
        }
    }
    ...
    let telemetry_services: Vec<_> = report
        .services
        .iter()
        .filter(|s| matches!(s.category, pieuvre_common::ServiceCategory::Telemetry))
        .collect();
    let running_telemetry = telemetry_services
        .iter()
        .filter(|s| matches!(s.status, pieuvre_common::ServiceStatus::Running))
        .count();

    println!(
        "  Télémétrie: {} services ({} running)",
        telemetry_services.len(),
        running_telemetry
    );

    let perf_services: Vec<_> = report
        .services
        .iter()
    ...
    let safe_to_remove: Vec<_> = report
        .appx
        .iter()
    ...
    let report_path =
        PathBuf::from(OUTPUT_DIR).join(format!("analyze_{}_{}.json", profile, timestamp));
