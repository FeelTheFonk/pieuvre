...
pub fn get_defender_status() -> Result<DefenderStatus> {
    let status = DefenderStatus {
        ...
        // PUA protection
        pua_protection: read_dword_value(r"SOFTWARE\Microsoft\Windows Defender", "PUAProtection")
            .unwrap_or(0)
            != 0,
...
pub fn get_firewall_status() -> Result<FirewallStatus> {
    ...
    Ok(FirewallStatus {
        domain_enabled: read_dword_value(&format!("{}\\DomainProfile", base), "EnableFirewall")
            .unwrap_or(1)
            == 1,

        private_enabled: read_dword_value(&format!("{}\\StandardProfile", base), "EnableFirewall")
            .unwrap_or(1)
            == 1,

        public_enabled: read_dword_value(&format!("{}\\PublicProfile", base), "EnableFirewall")
            .unwrap_or(1)
            == 1,
