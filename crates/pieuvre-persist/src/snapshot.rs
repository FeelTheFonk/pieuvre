...
fn save_compressed(dir: &Path, id: &str, data: &[u8]) -> Result<()> {
    ...
    let compressed = zstd::encode_all(data, 3)
        .map_err(|e| PieuvreError::Io(std::io::Error::other(e)))?;
    ...
    if checksum_path.exists() {
        ...
        if expected.trim() != actual {
            return Err(PieuvreError::Parse(format!(
                "Checksum mismatch: expected {}, got {}", expected.trim(), actual
            )));
        }
...
fn rotate_snapshots(dir: &Path) -> Result<()> {
    ...
    tracing::info!(removed = to_remove, max = DEFAULT_MAX_SNAPSHOTS, "Rotation snapshots");
...
pub fn restore(id: &str) -> Result<()> {
    ...
    for change in &snapshot.changes {
        match change {
            ChangeRecord::Registry { key, value_name, value_type: _, original_data } => {
                ...
                if original_data.len() == 4 {
                    ...
                    match pieuvre_sync::registry::set_dword_value(key, value_name, value) {
                        Ok(_) => {
                            tracing::info!(key = key, value_name = value_name, value = value, "Registry restauré");
                            restored += 1;
                        }
                        Err(e) => {
            ...
            ChangeRecord::Service { name, original_start_type } => {
                tracing::debug!(service = name, start_type = original_start_type, "Restauration service");
                ...
                let result = match *original_start_type {
                    ...
                    _ => {
                        tracing::warn!(service = name, start_type = original_start_type, "Start type non supporté");
                        Ok(())
                    }
                };
                ...
                match result {
                    Ok(_) => {
                        tracing::info!(service = name, start_type = original_start_type, "Service restauré");
                        restored += 1;
                    }
                    Err(e) => {
    ...
    tracing::info!(restored = restored, errors = errors, "Restauration terminée");
