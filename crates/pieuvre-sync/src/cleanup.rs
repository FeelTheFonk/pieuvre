...
/// Helper pour nettoyer les fichiers temporaires
...
/// Helper pour nettoyer WinSxS
pub fn cleanup_winsxs() -> Result<()> {
    let _ = std::process::Command::new("dism.exe")
        .args([
            "/online",
            "/cleanup-image",
...
/// Helper pour nettoyer le cache Edge
pub fn cleanup_edge_cache() -> Result<()> {
    if let Some(local_appdata) = std::env::var_os("LOCALAPPDATA") {
        let edge_cache =
            PathBuf::from(local_appdata).join(r"Microsoft\Edge\User Data\Default\Cache");
