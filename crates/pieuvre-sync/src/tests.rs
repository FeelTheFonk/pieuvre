...
use crate::{cpu, dpc, game_mode, power, security, services, timer};
...
fn test_timer_resolution_info_conversions() {
    ...
    assert!(
        (info.current_ms() - 1.0).abs() < 0.001,
        "Current should be 1.0ms"
    );
    assert!(
        (info.min_ms() - 15.625).abs() < 0.001,
        "Min should be 15.625ms"
    );
...
fn test_power_plan_guids() {
    assert_eq!(
        power::PowerPlan::Balanced.guid(),
        "381b4222-f694-41f0-9685-ff5bb260df2e"
    );
    assert_eq!(
        power::PowerPlan::HighPerformance.guid(),
        "8c5e7fda-e8bf-4a96-9a85-a6e23a8c635c"
    );
    assert_eq!(
        power::PowerPlan::UltimatePerformance.guid(),
...
fn test_registry_key_paths_valid() {
    ...
    for path in valid_paths {
        ...
        assert!(
            !path.ends_with('\\'),
            "Path should not end with backslash: {}",
            path
        );
    }
}
...
fn test_profile_names_valid() {
    ...
    for profile in valid_profiles {
        ...
        assert!(
            profile.chars().all(|c| c.is_ascii_lowercase()),
            "Profile name should be lowercase: {}",
            profile
        );
    }
}
...
async fn test_apply_profile_dry_run_unknown() {
    ...
    assert!(
        result.is_ok(),
        "Dry-run unknown profile should succeed (no-op)"
    );
}
...
fn test_telemetry_services_list() {
    ...
    for svc in telemetry_services {
        assert!(!svc.is_empty());
        assert!(
            svc.chars().all(|c| c.is_ascii_alphanumeric()),
...
fn test_power_plan_names() {
    ...
    assert_eq!(
        power::PowerPlan::UltimatePerformance.name(),
...
fn test_unicode_service_name() {
    ...
    assert!(
        result.is_err(),
        "Unicode service name should fail gracefully"
    );
}
